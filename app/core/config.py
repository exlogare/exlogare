...
class Settings(BaseSettings):
    ...
    # Empty → derived from PUBLIC_BASE_URL (see _default_oauth_redirects).
    ...
    def _default_oauth_redirects(self) -> Settings:
        """Derive OAuth callback URLs from PUBLIC_BASE_URL when unset."""
        base = self.public_base_url.rstrip("/")
        defaults = {
            "gitlab_oauth_redirect_uri": f"{base}/auth/gitlab/callback",
            "github_oauth_redirect_uri": f"{base}/api/integrations/github/oauth/callback",
            "bitbucket_oauth_redirect_uri": (
                f"{base}/api/integrations/bitbucket/oauth/callback"
            ),
            "gitflic_oauth_redirect_uri": f"{base}/api/integrations/gitflic/oauth/callback",
            "oidc_redirect_uri": f"{base}/api/auth/oidc/callback",
        }
        for field, derived in defaults.items():
