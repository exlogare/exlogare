...
OAuth callback URLs (`GITLAB_OAUTH_REDIRECT_URI`, GitHub/Bitbucket/GitFlic, OIDC) default to paths under `PUBLIC_BASE_URL` when left empty. Put the same URL into the GitLab/GitHub OAuth Application — do **not** register `localhost` if your Git host cannot reach it; set `PUBLIC_BASE_URL` to a LAN IP, DDNS, or tunnel instead.
...
| `GITLAB_OAUTH_REDIRECT_URI` | `{PUBLIC_BASE_URL}/auth/gitlab/callback` | Override only if needed; leave empty to follow `PUBLIC_BASE_URL`. |
...
| `GITHUB_OAUTH_REDIRECT_URI` | `{PUBLIC_BASE_URL}/api/integrations/github/oauth/callback` | Leave empty to follow `PUBLIC_BASE_URL`. |
...
| `BITBUCKET_OAUTH_REDIRECT_URI` | `{PUBLIC_BASE_URL}/api/integrations/bitbucket/oauth/callback` | Leave empty to follow `PUBLIC_BASE_URL`. |
...
| `GITFLIC_OAUTH_REDIRECT_URI` | `{PUBLIC_BASE_URL}/api/integrations/gitflic/oauth/callback` | Leave empty to follow `PUBLIC_BASE_URL`. |
